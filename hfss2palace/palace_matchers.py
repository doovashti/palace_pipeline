...
REVISION (this version): the deferred pass of
match_surface_conductor_surfaces no longer demands exactly ONE
unclaimed plausible descendant per original conductor face. A face
clipped by a butt-jointed solid (an SMA pin end cap pressed against its
wirebond pad) legitimately shatters into SEVERAL disjoint exposed
remnants; the whole set is now accepted when every piece lies inside
the original face's bounding box and their combined area does not
exceed the original face's area. A set whose total area exceeds the
original still fails loudly.
"""
...
def match_surface_conductor_surfaces(
    ...
    face. A face CLIPPED by a butt-jointed solid (part buried, part
    exposed) may leave several disjoint remnants whose areas do NOT sum
    to the original; the deferred pass accepts that whole set when it
    fits inside the original face's bounding box with total area at
    most the original's.
    ...
    for body_name, conductor_record in conductor_records.items():
        ...
        # own clipped remnant(s) (accept), a set too large to be its
        # remnants (refuse), or nothing (skip -- the face vanished
        # into a wall).
        for face_record in deferred_faces:
            ...
            # CLIPPED / SHATTERED-FACE CASE. By this point every
            # sibling face of this body (and every earlier-matched
            # boundary) has claimed its surfaces. What remains inside
            # this face's bounding box is the set of exposed remnants
            # the booleans left behind. That set is legitimately
            # PLURAL: a conductor face butt-jointed against another
            # solid is clipped by the contact patch into several
            # disjoint pieces -- an SMA pin end cap pressed against
            # its wirebond pad leaves exposed crescents above and
            # below the pad -- and the later sheet-imprint
            # fragmentation can split those pieces further. Demanding
            # exactly one candidate here (the old behavior) was
            # wrong. Accept the whole set when it is geometrically
            # credible: every piece lies inside the original face's
            # bounding box (already enforced by the `plausible`
            # filter) and the combined area does not exceed the
            # original face's area. A set whose total area EXCEEDS
            # the original cannot be its remnants, so that still
            # fails loudly.
            #
            # Attribution caveat: a remnant of a TOUCHING body's
            # coplanar face (the pad's own end face, say) can land in
            # this set. That is harmless when the touching bodies
            # share a material -- both carry the identical
            # finite-conductivity boundary, and the touching body's
            # own face pass later finds its surfaces claimed and
            # skips them. Butt-jointed conductors of DIFFERENT
            # materials could mis-attribute a sliver's sigma; check
            # the printed areas if you ever build one.
            total_area = sum(float(item["area"]) for item in plausible)
            retained_fraction = total_area / max(
                target_area, CONDUCTOR_AREA_ABS_TOL_MM2
            )

            if total_area > target_area + area_tolerance:
                raise RuntimeError(
                    f"Conductor '{body_name}' original surface "
                    f"{face_record['surface']} (area "
                    f"{target_area:.12g} mm^2, center "
                    f"{target_center}) has {len(plausible)} unclaimed "
                    f"final surfaces inside its footprint "
                    f"({[item['surface'] for item in plausible]}) "
                    f"with combined area {total_area:.12g} mm^2 -- "
                    f"MORE than the original face. They cannot all "
                    f"be its remnants. Refusing to guess."
                )

            print(
                f"  original conductor surface "
                f"{face_record['surface']} was clipped/split by "
                f"booleans; accepting {len(plausible)} exposed "
                f"remnant(s) with combined area={total_area:.12g} "
                f"mm^2 ({100.0 * retained_fraction:.6g}% of the "
                f"original {target_area:.12g} mm^2)"
            )

            for item in plausible:
                if item["surface"] in excluded_surfaces:
                    raise RuntimeError(
                        f"Conductor surface {item['surface']} for "
                        f"'{body_name}' is already reserved for another "
                        "boundary"
                    )

                body_matches.add(item["surface"])
                already_claimed.add(item["surface"])
                print(
                    f"  original surface={face_record['surface']} "
                    f"-> final surface={item['surface']}, "
                    f"center={item['center']}, "
                    f"area={item['area']:.12g} mm^2, "
                    f"adjacent_volumes={item['adjacent']}"
                )
